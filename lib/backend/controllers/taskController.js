...
async function getProductsForObject(req, res) {
  try {
    ...
    const products = objectPart.products.map((p) => {
      const prodDoc = p.productId && p.productId._id ? p.productId : null;
      return {
        _id: prodDoc ? prodDoc._id : p.productId,
        productId: prodDoc ? prodDoc._id : p.productId,
        name: prodDoc ? prodDoc.name : undefined, // локализуемое поле
        status: p.status,
        price: p.price,
        photoUrl: p.photoUrl,
      };

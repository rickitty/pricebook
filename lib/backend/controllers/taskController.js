...
async function getProductsForObject(req, res) {
  try {
    ...
    if (dist > MAX_DIST_METERS) {
      ...
      const products = objectPart.products.map((p) => {
      const prod = p.productId && p.productId._id ? p.productId : null;

      return {
        _id: prod?._id ?? p.productId,
        productId: prod?._id ?? p.productId,

        // Локализуемые поля
        name: prod?.name,
        category: prod?.category,  
        imageUrl: prod?.imageUrl, // <-- фото из products коллекции
        iconUrl: prod?.iconUrl,
        // Данные задачи
        status: p.status,
        price: p.price,
        photoUrl: p.photoUrl,        // фото, которое пользователь делает в задаче
      };
    });
